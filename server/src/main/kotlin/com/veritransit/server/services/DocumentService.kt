 ...
 * must carry a human who confirmed them on screen. [attach] *enforces* that for
 * the finance-critical kinds (PO, INVOICE, EWB) — a machine-read document with
 * no confirming human is rejected rather than stored unconfirmed, because the
 * matcher cannot tell a confirmed number from an unconfirmed one after the fact.
 */
class DocumentService(private val db: Database, private val audit: AuditLog) {
    ...
    fun attach(
        ...
        // §5.1/§12 — the safety rail is a requirement, not metadata: whatever
        // reads an invoice reads it, but only a confirmed one gets to move money.
        val confirming = confirmedBy?.trim()?.takeIf { it.isNotEmpty() }
        if (doc.kind in FINANCE_CRITICAL && confirming == null) {
            throw IllegalArgumentException(
                "${doc.kind.name} ${doc.docNo} drives payment — attach it with the " +
                    "X-Confirmed-By header naming the human who checked the numbers on screen",
            )
        }
        ...
        conn.update(
            ...
            doc.confidence, confirming, uploadedBy, ref,
        )
        ...
        audit.append(conn, uploadedBy ?: "system", "DOCUMENT_ATTACHED", ref, buildJsonObject {
            ...
            put("confirmed_by", confirming ?: "")
        })

        doc.copy(shipmentRef = ref, confirmedBy = confirming)
    }
    ...
    /**
     * The order value is derived from *confirmed* paperwork only — an unconfirmed
     * machine reading must never set how much money a shipment is worth.
     */
    fun orderValue(ref: String): Double? = db.queryOne(
        """SELECT COALESCE(
                    (SELECT (fact -> 'totals' ->> 'taxable_value')::numeric FROM documents d
                       JOIN shipments s ON s.id = d.shipment_id
                      WHERE s.ref = ? AND d.kind = 'INVOICE' AND d.confirmed_by IS NOT NULL
                      ORDER BY d.uploaded_at DESC LIMIT 1),
                    (SELECT (fact -> 'totals' ->> 'taxable_value')::numeric FROM documents d
                       JOIN shipments s ON s.id = d.shipment_id
                      WHERE s.ref = ? AND d.kind = 'PO' AND d.confirmed_by IS NOT NULL
                      ORDER BY d.uploaded_at DESC LIMIT 1)
                  ) AS v""",
        ref, ref,
    ) { it.dblOrNull("v") }
    ...
    companion object {
        /** §5.1 kinds whose numbers move money; [attach] demands a `confirmed_by`. */
        val FINANCE_CRITICAL = setOf(DocumentKind.PO, DocumentKind.INVOICE, DocumentKind.EWB)
    }
}
