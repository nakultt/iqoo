...
/**
 * Seed receiving records — mirrors the demo log in the receiving app's `Repo`
 * so the bot and the app tell the same story for every delivery.
 */
object BotData {

    const val WAREHOUSE = "Central Warehouse"
    const val RECEIVER = "Goods-In Clerk"
    const val DOCK = "Dock 3 · Central DC"

    fun seed(now: Long): List<ReceivingRecord> {
